...
export class PaginaPrincipalComponent implements OnInit {
  ...
  addFavoritos(coffe: string){
    ...
    if (this.pedidoServicio.logeado.nombre != undefined){
        ...
        if (this.cafes[i].typeCoffe == coffe){
          ...
          this.pedidoServicio.fav(this.favori).subscribe(data=> this.pedidoServicio.favoritos = data);
          this.openaddFavoritos(this.cafes[i].typeCoffe);
          break;
        }
      }
    }else{
  ...
  openaddFavoritos(tipoFav: String) {
    let cont = 0;
    for(let i = 0; i < this.pedidoServicio.favoritos.length; i++){
      if (tipoFav == this.pedidoServicio.favoritos[i].typeCoffe){
        cont = 1;
      }
    }
    if (cont == 1){
      this._snackBar.open( String (tipoFav),"Quitado de Favoritos",{duration: 1000, horizontalPosition:'end', verticalPosition: 'bottom'});
    }else {
      this._snackBar.open( String (tipoFav),"Añadido",{duration: 1000, horizontalPosition:'end', verticalPosition: 'bottom'});
    }
  }
}
